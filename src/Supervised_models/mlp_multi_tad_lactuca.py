...
def run_mlp(datapath, dataset, seed=0,logging_dir='logs/model',
    ...
    # Train the model
    name = '_'.join(dataset.split('/')[-1].split('_')[:-1])
    ...
    # Save the weights
    model.save_weights(os.path.join(datapath,f"weights/mlp_weights_{name}_lr{learning_rate}_drop{dropout}_ls{layersize}"))

    if eval_model:
        model_performance(model,valid_x,valid_y,datapath, name, learning_rate)




def model_performance(model,x,y,path=None, name=None, lr=None):
    ...
    roc_auc = dict()
    for i in range(4):
        fpr_mlp[i], tpr_mlp[i], _ = roc_curve(y[:, i], y_pred_mlp[:, i])
        roc_auc[i] = auc(fpr_mlp[i], tpr_mlp[i])
    ...
    if path is not None:
        ...
        plot_roc(fpr_mlp, tpr_mlp, roc_auc, name, lr, path)
    print('*******' * 3, '\n\t AUC: ', roc_auc, '\n', '*******' * 3)


def plot_roc(fpr, tpr, roc_auc, name, lr, path):
    fig, ax = plt.subplots()
...
def main(features, lr=0.001, dropout=0.0, layersize=512):
    ...
    datapath = os.path.join(os.path.dirname(features), 'mlp_multi_allWindows')#{}'.format(now))
