...
def model_performance(model, x, y, path=None, params=None, name=None):
    ...
    roc_auc = dict()
    for i in range(4):
        fpr_mlp[i], tpr_mlp[i], _ = roc_curve(y[:, i], y_pred_rf[i])
...
def plot_roc(fpr, tpr, roc_auc, name, params, path):
    ...
    ax.legend()
    file_name = f"{name}_ne{params['n_estimators']}_md{params['max_depth']}_ml{params['min_samples_leaf']}_roc.png"
    plt.savefig(os.path.join(path, file_name))

def main(features, params):
    name = features.split("/")[-1].split(".csv")[0]
    now = datetime.datetime.now().strftime("%Y%m%d")
    datapath = os.path.join(os.path.dirname(features), 'rf_multi_allWindows_new')#{}'.format(now))
...
if __name__ == '__main__':
    ...
    now = datetime.datetime.now().strftime("%Y%m%d")
    datapath = os.path.join(dir, 'rf_multi_rf_multi_allWindows_new')#{}'.format(now))
    os.makedirs(datapath,exist_ok=True)
